...
static int __init etx_driver_init(void)
{
	...
	if ((alloc_chrdev_region(&dev, 0, 1, "etx_Dev_High_Resolution_Timer")) < 0)
	{
	...
	if (IS_ERR(dev_class = class_create(THIS_MODULE, "etx_class_high_resolution_timer")))
	{
	...
	/*Creating device*/
	if (IS_ERR(device_create(dev_class, NULL, dev, NULL, "etx_device_high_resolution_timer")))
	{
